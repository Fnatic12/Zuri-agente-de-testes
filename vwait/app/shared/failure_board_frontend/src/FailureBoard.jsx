...
function BoardCard({
  ...
  const handleOpenJiraClick = (event) => {
    event.preventDefault();
    event.stopPropagation();
    onOpenJiraClick(item.id, item.jiraUrl);
  };
  ...
  return (
    ...
    >
        ...
        <div className="failure-card-actions">
          {item.jiraUrl ? (
            <button
              ...
              draggable={false}
              onClick={handleOpenJiraClick}
              onMouseDown={stopCardDrag}
              onPointerDown={stopCardDrag}
              title={item.jiraIssueKey ? `Abrir ${item.jiraIssueKey} no Jira` : "Abrir no Jira"}
              type="button"
            >
              Jira
            </button>
      ...
      <div className="failure-card-content">
        {item.jiraIssueKey ? (
          <div className="failure-card-issue-key" title={`Issue vinculada: ${item.jiraIssueKey}`}>
            {item.jiraIssueKey}
          </div>
        ) : null}
        ...
        {items.map((item, index) => (
          <Fragment key={item.id}>
            ...
            <BoardCard
              ...
              onOpenJiraClick={onOpenJiraClick}
...
function FailureBoardComponent(props) {
  ...
  const handleOpenJiraClick = (itemId, jiraUrl) => {
    if (!jiraUrl) {
      return;
    }
    window.open(jiraUrl, "_blank", "noopener,noreferrer");
    pushValue(buildPayload("open_jira", containers, itemId));
  };
      ...
      <div className="board-grid" ref={gridRef}>
        {containers.map((container) => (
          <Lane
            ...
            onOpenJiraClick={handleOpenJiraClick}
