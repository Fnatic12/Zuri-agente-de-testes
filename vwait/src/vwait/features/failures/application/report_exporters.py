...
def make_report_dir(category: str, test_name: str, generated_at: str, reports_root: Path | None = None) -> Path:
    safe_stamp = generated_at.replace(":", "-")
    base_dir = Path(reports_root) if reports_root is not None else ensure_reports_dir()
    out_dir = base_dir / category / test_name / safe_stamp
