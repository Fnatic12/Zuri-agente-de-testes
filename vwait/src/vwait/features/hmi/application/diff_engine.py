...
def compare_images(imgA: np.ndarray, imgB: np.ndarray, config: DiffConfig) -> dict[str, Any]:
    ...
    for bbox, score in bboxes:
        ...
        if _is_toggle_candidate(bbox, config):
            ...
            state_a_k, _ = _toggle_state_by_knob(roi_a)
            state_b_k, _ = _toggle_state_by_knob(roi_b)
        ...
        diffs.append({"bbox": bbox, "score": float(score), "type": dtype})
    ...
    return {
        ...
        "debug_images": {"diff_mask": mask, "overlay": overlay},
    }


__all__ = ["DiffConfig", "compare_images"]
